...
| `investigate_resource` | exact `resourceId` | subject, changes, native evidence, related context, known facts, missing context, provider activity, timeline, exact shared-commit groups, retained resolution memory for that exact subject (organizational response, not current provider truth, not a recommendation; omitted when none), and retained incident grouping for that exact subject (organizational grouping, not current provider truth, not a recommendation; omitted when none) |
