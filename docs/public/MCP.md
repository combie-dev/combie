...
- `response-recall` — retained `investigationHistory`, `resolutionMemory`,
  `incidentMemory`, and additive `structuredResponseMemory` (always present;
  `[]` is known-empty). Exact Recommendation→Decision→Action→Outcome chains
  for the subject; organizational memory, not current provider truth, not
  inferred Action, not a replacement of `resolutionMemory`. Exact prior
...
is no sixth tool. Recommendation, Decision, Action, and Outcome recording
stays CLI-only.
