...
### Task-scoped investigate (optional `task`)

`investigate_resource` accepts an optional `task` enum (`change-review`,
`dependency-impact`, `response-recall`) that returns a smaller deterministic
view of the same live compose plus the same exact-subject Investigation /
Resolution / Incident reads. The result opens with a `task` envelope
(`profile`, `subjectResourceId`) and a `subject`; the selected sections differ
by profile, but field shapes are unchanged from the full investigation.

- `change-review` — subject authority, changes, one-hop evidence, provider
  activity, timeline, shared commits, relationships/paths, evidence gaps.
  Chronology and correspondence, not causality.
- `dependency-impact` — direct Relationships, directions, neighbors, two-hop
  paths, relationship evidence and verification clocks, graph/discovery gaps.
  Connectivity, not blast radius.
- `response-recall` — retained `investigationHistory`, `resolutionMemory`,
  `incidentMemory`, additive `structuredResponseMemory` (always present;
  `[]` is known-empty), additive `incidentPrecedentMemory` (always present;
  `[]` is known-empty), and additive `incidentResponseExperienceMemory`
  (always present; `[]` is known-empty). Exact Recommendation→Decision→Action→Outcome
  chains for the subject; organizational memory, not current provider truth,
  not inferred Action, not a replacement of `resolutionMemory`. One precedent
  set per exact Incident already in `incidentMemory` (explicit human/agent
  links and deterministic exact-match candidates — not recommendations, not
  similarity). Exact temporally prior records only
  (`effectiveAt = occurredAt ?? recordedAt`; peer must be strictly before the
  query). Equal or later Incidents are never precedents — including explicit
  link peers (those links remain on `incident-links`). Not similar incidents.
  `incidentResponseExperienceMemory` is one per-`actionKey` summary over those
  exact prior Incident-anchored chains, keeping PROPOSED (Recommendations and
  Decisions by literal disposition) separate from ATTEMPTED (Actions and
  Outcomes by literal assessment); every count is backed by exact retained
  ids, assessments are recorded not computed (no success, rank, or
  recommendation), incomplete branches (Recommendation without Decision,
  Action without Outcome) are named, and only temporally prior precedents
  contribute. No new tool or write.

`task` requires a non-empty `resourceId`, cannot be combined with
`investigationId`, is read-only, never calls providers, and does not change
database bytes. Omitted `task` returns the full investigation context
unchanged.

### Progressive disclosure: `availableOnDemand`

Every task-scoped result adds one top-level `availableOnDemand` array of inert
retrieval descriptors — always present on task results, never on omitted-task
full investigate. Combie never executes them; the caller decides whether to
follow one. Ordering: exactly one `current-investigation` first, then
`retained-investigation` entries in investigation-history order (only on
`response-recall`; `change-review` and `dependency-impact` expose only the one
current target because those profiles exclude memory).

- `current-investigation` — for the exact subject, a new live local compose.
  `cli.argv: ["combie", "investigate", "<resource-id>", "--json"]`, or MCP
  `investigate_resource` with `arguments: { resourceId: "<resource-id>" }` (no
  `task`). Not frozen to the earlier task call; a sync between calls may change
  evidence and clocks.
- `retained-investigation` — `kind`, `investigationId`, `subjectResourceId`,
  `composedAt`. `cli.argv: ["combie", "investigation", "<investigationId>"]`
  retrieves the complete retained (frozen at `composedAt`) composition; MCP
  `investigate_resource` with `arguments: { investigationId: "<id>" }` and
  `returns: "retained-snapshot-handle"` — the existing thin named-id boundary
  (snapshot identity + bounded subject preview + artifact handle), not the
  snapshot body.
...
is no sixth tool. Recommendation, Decision, Action, Outcome, and incident-link
recording stays CLI-only. Direct precedent retrieval is CLI
`precedents --incident` (human or `--json`); MCP observes the same projection
only as additive `incidentPrecedentMemory` on `response-recall`.
